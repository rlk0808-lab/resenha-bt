  ...
  async function calcularBadges(rodadaId, ranking) {
    ...
    async function expandirFormato(novoFormato) {
      ...
      const liga = await buscarLigaAtual();
      let query = supabase.from("rodadas").select("id").in("status", ["ativa", "proxima"]).order("numero", { ascending: true }).limit(1);
      if (liga) query = query.eq("liga", liga);
      const { data: aberta } = await query;
      if (aberta?.[0]) {
        await supabase.from("rodadas").update({ vagas_total: novoFormato.total }).eq("id", aberta[0].id);
        await carregarRodadas();
      }
